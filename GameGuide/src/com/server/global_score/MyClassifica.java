...
public class MyClassifica {


		SQLiteDatabase mDb;
		DbHelper mDbHelper;
		Context mContext;
		private static final String DB_NAME="classifica";//nome del db
		private static final int DB_VERSION=1; //numero di versione del nostro db
		
		public MyClassifica(Context ctx){
			mContext=ctx;
	...
	//Chiudo il cursore
	 if (cursor != null && !cursor.isClosed())
		...
		public String getComment1(String nome){
			...
			//this.startManagingCursor(cursor);
			int nome_int=cursor.getColumnIndex(MyClassifica.LoginMetaData.NOME_UTENTE);
		      int commento1=cursor.getColumnIndex(MyClassifica.LoginMetaData.COMMENTO1);
			  if(cursor.moveToFirst()) {
				  do {
					  if(cursor.getString(nome_int).equalsIgnoreCase(nome))
	...
	//Chiudo il cursore
	  if (cursor != null && !cursor.isClosed())
		...
		public String getComment2(String nome){
			...
			//this.startManagingCursor(cursor);
			int nome_int=cursor.getColumnIndex(MyClassifica.LoginMetaData.NOME_UTENTE);
		      int commento2=cursor.getColumnIndex(MyClassifica.LoginMetaData.COMMENTO2);
			  if(cursor.moveToFirst()) {
				  do {
					  if(cursor.getString(nome_int).equalsIgnoreCase(nome))
	...
	//Chiudo il cursore
	 if (cursor != null && !cursor.isClosed())
		...
		public String getData(String nome){
			...
			//this.startManagingCursor(cursor);
			int nome_int=cursor.getColumnIndex(MyClassifica.LoginMetaData.NOME_UTENTE);
		      int data=cursor.getColumnIndex(MyClassifica.LoginMetaData.DATA);
			  if(cursor.moveToFirst()) {
				  do {
					  if(cursor.getString(nome_int).equalsIgnoreCase(nome))
	...
	//Chiudo il cursore
	 if (cursor != null && !cursor.isClosed())
		...
		public int recupera_id(String nome){
			...
			//this.startManagingCursor(cursor);
			int nome_int=cursor.getColumnIndex(MyClassifica.LoginMetaData.NOME_UTENTE);
		      int id_utente=cursor.getColumnIndex(MyClassifica.LoginMetaData.ID);
			  if(cursor.moveToFirst()) {
				  do {
	 ...
	 // Chiudo il cursore
		...
		public int get_fine_gioco(String nome){
			...
			//this.startManagingCursor(cursor);
			int nome_int=cursor.getColumnIndex(MyClassifica.LoginMetaData.NOME_UTENTE);
		      int flag_fine_gioco=cursor.getColumnIndex(MyClassifica.LoginMetaData.FINE_GIOCO);
			  if(cursor.moveToFirst()) {
				  do {
					  if(cursor.getString(nome_int).equalsIgnoreCase(nome))
	...
	public	static class LoginMetaData {  // i metadati della tabella, accessibili ovunque
			static final String LOGIN_TABLE = "classifica";
